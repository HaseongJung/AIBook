...
torch.backends.cudnn.benchmark = True  # 성능 향상을 위해 cudnn.benchmark 활성화
torch.backends.cudnn.max_workspace = 1024 * 1024 * 1024  # 1GB로 max_workspace 설정
...
sd_refiner = StableDiffusionXLImg2ImgPipeline.from_pretrained("stabilityai/stable-diffusion-xl-refiner-1.0",
...
def generate_book(main_char, subject):
    ...
    medium = ['colored pencil', "Sebastian, children's book illustrator, Whimsical and colorful style, Medium: Watercolor, Color Scheme: Bright and lively color palette",'cinematic,character design by mark ryden and pixar and hayao miyazaki, unreal 5,', 'large pastel, a color pencil sketch inspired by Edwin Landseer', 'a storybook illustration by Marten Post']
